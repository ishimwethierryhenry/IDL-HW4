...
class SelfAttentionDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, d_ff: int, dropout: float = 0.1):
        ...
        # one self-attention sublayer and one feedforward sublayer
        # that's all a GPT-style decoder layer needs
    ...
    def forward(self, x: torch.Tensor, key_padding_mask: Optional[torch.Tensor] = None, attn_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        '''
        # first pass through masked self-attention
        # causal mask prevents each token from seeing future positions
        x, handel_attn_weights = self.self_attn(
            x,
            key_padding_mask=key_padding_mask,
            attn_mask=attn_mask
        )

        # then through feedforward
        x = self.ffn(x)

        return x, handel_attn_weights
...
class CrossAttentionDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, d_ff: int, dropout: float = 0.1):
        ...
        # decoder-side self-attention (masked so decoder can't peek ahead)
        self.self_attn  = SelfAttentionLayer(d_model, num_heads, dropout)
    ...
    def forward(self, x: torch.Tensor, enc_output: torch.Tensor, dec_key_padding_mask: Optional[torch.Tensor] = None, enc_key_padding_mask: Optional[torch.Tensor] = None, attn_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        x, handel_self_attn_weights = self.self_attn(
            x,
            key_padding_mask=dec_key_padding_mask,
            attn_mask=attn_mask
        )

        # step 2: cross-attention -- decoder queries, encoder key/value
        # enc_key_padding_mask masks out padding in the encoder output
        x, henry_cross_attn_weights = self.cross_attn(
            x,
            enc_output,
        ...
        return x, handel_self_attn_weights, henry_cross_attn_weights
## -------------------------------------------------------------------------------------------------    
