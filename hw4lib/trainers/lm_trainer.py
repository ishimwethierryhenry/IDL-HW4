...
class LMTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        # CrossEntropyLoss with ignore_index so pad tokens don't contribute to the loss
        # label_smoothing from config helps prevent overconfident predictions
    ...
    def _train_epoch(self, dataloader) -> Tuple[Dict[str, float], Dict[str, torch.Tensor]]:
        ...
        for i, batch in enumerate(dataloader):
            # unpack batch and move everything to the training device
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                # CrossEntropyLoss expects (B, vocab_size, T) or we can flatten to (B*T, vocab_size)
                # targets_golden needs to be (B*T,) accordingly
            ...
            # scale the loss and backprop through it
    ...
    def _validate_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.inference_mode():
                # forward pass -- no gradients needed during validation
                raw_preds, attn_weights = self.model(targets_shifted, lengths)
    ...
    def train(self, train_dataloader, val_dataloader, epochs: int):
        ...
        for epoch in range(self.current_epoch, self.current_epoch + epochs):
            ...
            # generate some sample text to see qualitatively how the model is doing
    ...
    def generate(self, dataloader, generation_config: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        with torch.inference_mode():
            if generation_config.get('top_k', 0) > 0 or generation_config.get('top_p', 0) > 0:
                ...
                # use the greedy generator we implemented in SequenceGenerator
