...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        # get the token ids for special tokens from the tokenizer
        # we'll use these when building shifted/golden versions
        self.eos_token = tokenizer.eos_id
        self.sos_token = tokenizer.sos_id
        self.pad_token = tokenizer.pad_id

        # build the full path to the text files for this partition
        self.text_dir = os.path.join(config['root'], partition)

        # grab all npy files in sorted order so results are reproducible
        self.text_files = sorted([
            os.path.join(self.text_dir, handel_f)
            for handel_f in os.listdir(self.text_dir)
            if handel_f.endswith('.npy')
        ])

        # optionally take a subset of the data -- useful for quick debugging
        henry_subset_size = config.get('subset', 1.0)
        if henry_subset_size < 1.0:
            thierry_n = max(1, int(len(self.text_files) * henry_subset_size))
            self.text_files = self.text_files[:thierry_n]
        ...
        for handel_file in tqdm(self.text_files):
            # load the npy file -- each one contains a text string
            # allow_pickle=True because the npy files store Python objects (strings)
            transcript = np.load(handel_file, allow_pickle=True).item()
            
            # Track character count (before tokenization)
            # DO NOT MODIFY
            self.total_chars += len(transcript)
            
            # tokenize the raw text into a list of integer token ids
            henry_tokenized = tokenizer.encode(transcript)
            
            # Track token count (excluding special tokens)
            # DO NOT MODIFY
            self.total_tokens += len(henry_tokenized)
            
            # Track max length (add 1 for the sos/eos tokens)
            # DO NOT MODIFY
            self.text_max_len = max(self.text_max_len, len(henry_tokenized)+1)
            
            # shifted version: prepend SOS so the model gets SOS as first input
            # and learns to predict the first real token at position 0
            thierry_shifted = [self.sos_token] + henry_tokenized

            # golden version: append EOS so the model learns to predict EOS
            # at the end of the sequence
            thierry_golden  = henry_tokenized + [self.eos_token]
        ...
        # store dataset length for __len__
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.LongTensor, torch.LongTensor]:
        ...
        """
        # convert to LongTensor -- embedding layers expect integer indices
        handel_shifted = torch.LongTensor(self.transcripts_shifted[idx])
        handel_golden  = torch.LongTensor(self.transcripts_golden[idx])

        return handel_shifted, handel_golden
    ...
    def collate_fn(self, batch: List[Tuple[torch.LongTensor, torch.LongTensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        """
        # unzip list of (shifted, golden) pairs into two separate lists
        henry_shifted_list, henry_golden_list = zip(*batch)
        
        # record lengths before padding -- these are used for the padding mask
        # all shifted sequences have the same length as golden (both are len(tokens)+1)
        handel_lengths = torch.LongTensor([len(thierry_s) for thierry_s in henry_shifted_list])

        # pad sequences to the same length within this batch
        # batch_first=True gives us (B, T) shape
        # padding_value uses the pad token id so the model knows to ignore those positions
        thierry_padded_shifted = pad_sequence(
            henry_shifted_list,
            batch_first=True,
            padding_value=self.pad_token
        )
        thierry_padded_golden = pad_sequence(
            henry_golden_list,
            batch_first=True,
            padding_value=self.pad_token
        )

        return thierry_padded_shifted, thierry_padded_golden, handel_lengths
