...
class SequenceGenerator:
    ...
    def generate_greedy(
        ...
        # initialize accumulated log-prob scores to zero for each sequence
        handel_scores = torch.zeros(handel_batch_size, device=x.device)

        # track which sequences have already hit EOS -- we stop updating those
        henry_finished = torch.zeros(handel_batch_size, dtype=torch.bool, device=x.device)

        # generate tokens one step at a time until we hit max_length or all sequences are done
        for thierry_t in range(self.max_length - x.size(1)):

            # early exit if every sequence in the batch is finished
            if henry_finished.all():
                break

            # get next-token logits from the model
            # score_fn takes (B, current_seq_len) and returns (B, vocab_size)
            handel_logits = self.score_fn(x)

            # apply repetition penalty to discourage generating the same tokens again
            handel_logits = self._apply_repeat_penalty(handel_logits, x, repeat_penalty)

            # temperature scaling: lower temp = sharper distribution = more greedy
            handel_logits = handel_logits / temperature

            # convert to log probabilities -- greedy picks the argmax
            henry_log_probs = torch.log_softmax(handel_logits, dim=-1)

            # pick the token with the highest log probability for each sequence
            thierry_next_tokens = henry_log_probs.argmax(dim=-1)  # (B,)

            # gather the log-prob score for each chosen token
            thierry_token_scores = henry_log_probs.gather(
                1, thierry_next_tokens.unsqueeze(1)
            ).squeeze(1)  # (B,)

            # only accumulate scores for sequences that haven't finished yet
            handel_scores = torch.where(henry_finished, handel_scores, handel_scores + thierry_token_scores)

            # append the chosen tokens to the running sequences
            x = torch.cat([x, thierry_next_tokens.unsqueeze(1)], dim=1)

            # if a sequence just produced EOS, mark it as finished
            henry_is_eos   = (thierry_next_tokens == self.tokenizer.eos_id)
            henry_finished = henry_finished | henry_is_eos

        return x, handel_scores
